...
export interface PlayerHand { cards: Card[]; bet: number; stood?: boolean; busted?: boolean; surrendered?: boolean; doubled?: boolean; splitAces?: boolean; fromSplit?: boolean; insuranceBet?: number; settled?: number; }
...
export function dealInitial(state: RoundState): RoundState {
  ...
  const dealerBlackjack = isBlackjack(next.dealer);
...
export function legalActions(state: RoundState): PlayerAction[] {
  ...
  if (total >= 21 || h.stood || h.busted || h.surrendered) return [];
...
export function applyAction(state: RoundState, action: PlayerAction): RoundState {
  ...
  if (action==='hit') {
    let c; [c,next.shoe]=drawCard(next.shoe); h.cards.push(c);
    const total = handTotals(h.cards).total;
    if (total>21) h.busted=true;
    if (total===21) h.stood=true;
  }
  if (action==='double' && h.cards.length === 2 && !h.doubled && next.bankroll >= h.bet) {
    const originalBet = h.bet;
    let c;
    [c,next.shoe]=drawCard(next.shoe);
    h.cards.push(c);
    next.bankroll -= originalBet;
    h.bet = originalBet * 2;
    const total = handTotals(h.cards).total;
    if (total>21) h.busted=true;
    else h.stood=true;
  ...
  if (action==='surrender' && h.cards.length === 2) { h.surrendered=true; h.stood=true; }
  if (action==='split' && h.cards.length === 2 && isPair(h.cards) && next.hands.length < DEFAULT_RULES.maxHands && next.bankroll >= h.bet) {
    const [a,b]=h.cards;
    let c1,c2;
    [c1,next.shoe]=drawCard(next.shoe);
    [c2,next.shoe]=drawCard(next.shoe);
    const splittingAces = a.rank==='A';
    h.cards=[a,c1];
    h.fromSplit = true;
    h.splitAces = splittingAces;
    const splitHand: PlayerHand={cards:[b,c2],bet:h.bet,fromSplit:true,splitAces:splittingAces};
    next.hands.splice(next.currentHand+1,0,splitHand);
    next.bankroll -= h.bet;
    if (splittingAces) {
      if (handTotals(h.cards).total >= 21) h.stood = true;
    }
  }
  while (next.currentHand < next.hands.length && (next.hands[next.currentHand].stood || next.hands[next.currentHand].busted || next.hands[next.currentHand].surrendered)) next.currentHand +=1;
...
export function playDealer(state: RoundState): RoundState {
  ...
  while (true) {
    ...
    const mustHitSoft17 = DEFAULT_RULES.dealerHitsSoft17 && t.total === 17 && t.isSoft;
    if (!(t.total < 17 || mustHitSoft17)) break;
...
export function settle(state: RoundState): RoundState {
  ...
  for (const h of next.hands) {
    ...
    if (pt>21) continue;
    const naturalBlackjack = h.cards.length === 2 && pt === 21 && !h.fromSplit;
    if (naturalBlackjack) {
